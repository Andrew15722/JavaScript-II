...
let fullName = [];
runners.forEach(function(item, index) { // item acts just like runners 
    fullName.push(`${item.first_name} ${item.last_name}`);   // converted from concat to string interpolation
});
...
let allCaps = [];
runners.map(function(item, index) { // map and forEach are almost the same.
...
let largeShirts = [];
runners.filter(function(item, index) { // filter, map and forEach are almost the same.
    if(item.shirt_size === 'L') {
        largeShirts.push(`${item.first_name} ${item.last_name} needs a new size.`); // converted from concat to string interpolation
    }
});
...
runners.reduce(function(total, item, index, array) {
    total += item.donation;
    ticketPriceTotal.push(total);
    return total; // must return or the code will not work.
}, 0); // don't forget to add second argument
